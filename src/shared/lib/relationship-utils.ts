...
export const createRelationKeys = (prefix: string) => ({
  ...
  forParent: (parentId: Id) => [prefix, "relations", "parent", String(parentId)] as const,
  forChild: (childId: Id) => [prefix, "relations", "child", String(childId)] as const,
});

export const ensureIndex = (index: RelationIndex | undefined): RelationIndex =>
  index ?? { forward: {}, reverse: {} };

export const addRelation = (index: RelationIndex, parentId: Id, childId: Id): RelationIndex => {
...
export const removeRelation = (index: RelationIndex, parentId: Id, childId: Id): RelationIndex => {
  const p = String(parentId);
  const c = String(childId);
  const forward = { ...index.forward };
  const reverse = { ...index.reverse };
...
export const getChildren = (index: RelationIndex | undefined, parentId: Id): Id[] => {
  const i = ensureIndex(index);
  return i.forward[String(parentId)] ?? [];
};

export const getParents = (index: RelationIndex | undefined, childId: Id): Id[] => {
  const i = ensureIndex(index);
  return i.reverse[String(childId)] ?? [];
};

// Write-through helpers for React Query cache
export const setRelation = (qc: QueryClient, relationKey: QueryKey, parentId: Id, childId: Id) => {
  const prev = ensureIndex(qc.getQueryData<RelationIndex>(relationKey));
  qc.setQueryData<RelationIndex>(relationKey, addRelation(prev, parentId, childId));
  return prev;
};
...
export const unsetRelation = (
  ...
  const prev = ensureIndex(qc.getQueryData<RelationIndex>(relationKey));
  qc.setQueryData<RelationIndex>(relationKey, removeRelation(prev, parentId, childId));
  return prev;
};
